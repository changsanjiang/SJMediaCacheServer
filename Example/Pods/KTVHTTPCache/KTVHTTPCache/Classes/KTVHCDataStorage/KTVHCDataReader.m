...
#ifdef DEBUG
    printf("%s: <%p>.close { range: {%lld, %lld} };\n", NSStringFromClass(self.class).UTF8String, self, _request.range.start, _request.range.end + 1  - _request.range.start);
#endif
...
#ifdef DEBUG
    printf("%s: <%p>.createSubreaders { range: {%lld, %lld}, count: %lu };\n", NSStringFromClass(self.class).UTF8String, self, _request.range.start, _request.range.end + 1 - _request.range.start, sources.count);
#endif
