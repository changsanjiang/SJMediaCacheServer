...
#ifdef DEBUG
    printf("%s: <%p>.prepare { range: {%lld - %lld} };\n", NSStringFromClass(self.class).UTF8String, self, self.request.range.start, self.request.range.end);
#endif
...
#ifdef DEBUG
        printf("%s: <%p>.read { offset: %lld };\n", NSStringFromClass(self.class).UTF8String, self, _readedLength);
        
        if ( _finished ) {
            printf("%s: <%p>.done { range: {%lld, %lld} };\n", NSStringFromClass(self.class).UTF8String, self, _request.range.start, _request.range.end + 1 - _request.range.start);
        }
#endif
