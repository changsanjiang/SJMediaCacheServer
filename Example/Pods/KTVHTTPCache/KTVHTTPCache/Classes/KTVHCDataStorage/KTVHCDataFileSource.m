...
#ifdef DEBUG
    printf("%s: <%p>.prepare { range: {%lld, %lld} }, file: %s.{%lld, %lld} }; \n", NSStringFromClass(self.class).UTF8String, self, _range.start, _range.end + 1 - _range.start, _path.lastPathComponent.UTF8String, _readRange.start, _readRange.end + 1 - _readRange.start);
#endif
...
#ifdef DEBUG
        printf("KTVHCDataFileSource: <%p>.read { offset: %lld }; \n", self, _readedLength);
        
        if ( _finished ) {
            printf("KTVHCDataFileSource: <%p>.done { range: {%lld, %lld} }, file: %s.{%lld, %lld} }; \n", self, _range.start, _range.end + 1 - _range.start, _path.lastPathComponent.UTF8String, _readRange.start, _readRange.end + 1 - _readRange.start);
        }
#endif
